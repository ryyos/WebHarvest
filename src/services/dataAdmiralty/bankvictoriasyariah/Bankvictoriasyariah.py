...
from .dependency import BankvictoriasyariahLibs
from src.utils import *
from src.server import S3

class Bankvictoriasyariah(BankvictoriasyariahLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data, url in zip(self.type, datas, self.target_url):
            path: str = self.base_path+type+'_Bankvictoriasyariah.json'
            result = {
                "link": url,
                "type": type,
                "domain": self.domain,
    ...
    def main(self) -> None:
        ...
        for index, target in enumerate(self.target_url):
            ...
            if index == 0:
                results_direksi.extend(self.extract(html))
            else: 
                results_komisaris.extend(self.extract(html))
            ...
