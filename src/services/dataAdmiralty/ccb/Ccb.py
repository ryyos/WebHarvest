...
from .dependency import CcbLibs
...
class Ccb(CcbLibs):
    ...
    async def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zip(self.type, datas):
            path: str = self.base_path+type+'_ccb.json'
            result = {
    ...
    async def main(self) -> None:
        ...
        results_direksi: List[dict] = await self.extract(html.find('#direksi'))
        results_komisaris: List[dict] = await self.extract(html.find('#komisaris'))
