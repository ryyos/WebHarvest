...
from .dependency import PerdaniaLibs

class Perdania(PerdaniaLibs):
    ...
    async def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zip(self.type, datas):
            path: str = self.base_path+type+'_perdania.json'
            result = {
    ...
    async def main(self) -> None:
        ...
        for profile in html.find('table.resona-org-structure').eq(0).find('tr'):
            result: dict = await self.extract(PyQuery(profile))
            if 'Direktur' in result["nama_jabatan"]: results_direksi.append(result)
