...
from .dependency import MegasyariahLibs
from src.utils import *
from src.server import S3

class Megasyariah(MegasyariahLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data, url in zip(self.type, datas, self.target_url):
            path: str = self.base_path+type+'_megasyariah.json'
            result = {
                "link": url,
                "type": type,
                "domain": self.domain,
    ...
    def main(self) -> None:
        ...
        for index, target in enumerate(self.target_url):
            ...
            for profile in html.find('div[class="tabcontent-tentang-kami"] div[class="row"]'):
