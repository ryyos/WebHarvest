...
from .dependency import OkbankLibs
from src.utils import *
from src.server import S3

class Okbank(OkbankLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data, url in zip(self.type, datas, self.target_url):
            path: str = self.base_path+type+'_Okbank.json'
            result = {
                "link": url,
                "type": type,
                "domain": self.domain,
    ...
    def main(self) -> None:
        ...
        for index, target in enumerate(self.target_url):
            response: Response = self.api.get(target, cookies=self.cookies, headers=self.headers)
            
            for profile in response.json()["props"]["leaders"]["data"]:
                if index == 0: results_direksi.append(self.extract(profile))
                else: results_komisaris.append(self.extract(profile))
            ...
