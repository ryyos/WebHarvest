...
from .dependency import BjjLibs
...
class Bjj(BjjLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zip(self.type, datas):
            path: str = self.base_path+type+'_Bjj.json'
            result = {
    ...
    def main(self) -> None:
        ...
        for profile in html.find('div.dewan-box .ngc-media a'):
            results: dict = self.extract(PyQuery(profile).attr('data-src'))
