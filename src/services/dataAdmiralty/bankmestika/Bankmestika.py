...
from .dependency import BankmestikaLibs

class Bankmestika(BankmestikaLibs):
    ...
    async def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zippy(self.type, datas):
            path: str = self.base_path+type+'_bankmestika.json'
            result = {
    ...
    async def main(self) -> None:
        ...
        results_direksi: List[dict] = await self.extract(main_html=html,selector_html=html.find('#group1 div[class="struktur fl"] > p').eq(0))
        results_komisaris: List[dict] = await self.extract(main_html=html,selector_html=html.find('#group1 div[class="fl"] > p').eq(0))
