...
from .dependency import JagoLibs
...
class Jago(JagoLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zip(self.type, datas):
            path: str = self.base_path+type+'_Jago.json'
            result = {
    ...
    def main(self) -> None:
        response: Response = self.api.get(self.target_url, cookies=self.cookies, headers=self.headers)
        html: PyQuery = PyQuery(response.text)
        
        results_direksi: List[dict] = []
        results_komisaris: List[dict] = []

        for profile in html.find('#v-pills-boc > div.justify-content-between a'):
