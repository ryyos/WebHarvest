...
from .dependency import BtpnLibs
...
class Btpn(BtpnLibs):
    ...
    def chef(self, datas: Tuple[List[Dict[str, any]]]) -> None:
        ...
        for type, data in zip(self.type, datas):
            path: str = self.base_path+type+'_btpn.json'
            result = {
    ...
    def main(self) -> None:
        ...
        for komisaris in html.find('#komisaris a.director-col'):
            results_komisaris.append(self.extract(html.find(PyQuery(komisaris).attr('href'))))
            ...

        for direksi in html.find('#direktur a.director-col'):
            results_direksi.append(self.extract(html.find(PyQuery(direksi).attr('href'))))
            ...
